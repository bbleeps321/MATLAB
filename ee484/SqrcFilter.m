% SQRCFILTER    This function generates the filter coefficients for a
% Square root Raised Cosine (SQRC) filter.  The coefficients of the
...
% where n is based upon number of taps, Time delay, and samples
% per symbol.
    ...
    for TapIndex = 0:NumTaps-1
        ...
        elseif (abs(Time) == pi/(4*Alpha))
            Num = -((1+Alpha)/(4*Alpha))*sin((pi/4)*(1/Alpha+1)) - ...
                (1/pi)*sin((pi/4)*(1/Alpha-1))+ ((1- ...
                Alpha)/(4*Alpha))*cos((pi/4)*(1/Alpha-1));
            Den = -pi/(2*Alpha);
            Taps(TapIndex+1)= Num/Den;
        else
            Num = (1/(4*Alpha))*cos(Time*(1+Alpha)) + ...
                (pi/(16*(Alpha^2)*Time))*sin(Time*(1-Alpha));
