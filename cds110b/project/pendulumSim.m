...
%
%   DELAY will be approximated in the simulation based the time
%   discretization.
...
function timerFn(timerHandle, evt)
    ...
    title('Press Spacebar to end','Color','k');
    text(0,-.5,sprintf('t = %f,    x = %f,    theta = %f',...
        dt*ti,x(1),x(2)*180/pi),'HorizontalAlignment','center','Color','k');
return

function stopFn(timerHandle, evt)
    global data; global dt; global ti; global filename;
    global l; global l0; global mode; global M; global m;
    title('Done','Color','k');
    t = 0:dt:ti*dt;
    ...
    xhat = data(5:8,:);
    if (mode == 0)
        u = (M+m)*arrayfun(@(i) (data(3,i)-data(3,i-1))/dt,2:size(data,2));
    else
        u = data(9,:);
    end
...
function drawGraphics(x)
    ...
    line([xMin,xMax],[-cartHeight,-cartHeight],...
        'EraseMode','xor','LineStyle','-','Color','k');
