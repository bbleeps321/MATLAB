%% CNS 187 HW3 Problem 2.4 by Carey Zhang
% Based on RUN_EXPERIMENT.m by P. Perona - 16 October, 2013
% (c) California Institute of Technology

PARAMS.DT = 0.001; %% time bin size in seconds
PARAMS.T = 2; % length of observation in seconds
PARAMS.B = ceil(PARAMS.T/PARAMS.DT); % number of time bins in simulation
...
ylabel('number of decisions','FontSize',18);
title('C=0, C_{est}=0, T=[-1.2,1.2]','FontSize',16);

%% Threshold -1.8,1.8
MODEL.T = [-1.8 1.8]; % decision thresholds
...
RT10 = RT(find(C(:,2)==0),2); %% decision is 0 and class is 1
RT11 = RT(find(C(:,2)==1),2);
RT0 = RT(find(C==0)); % response times when underlying decision is 0
RT1 = RT(find(C==1)); % response times when the underlying decision is 1
N_AP0 = N_AP(find(C==0)); % n. action potentials when the decision was 0
N_AP1 = N_AP(find(C==1)); % n. action potentials when the decision was 1
...
xlabel('time (s)','FontSize',18);
ylabel('number of decisions','FontSize',18);
title('C=0, C_{est}=0, T=[-1.8,1.8]','FontSize',16);
...
legend(TAG,'Location','NE');
xlim([0 4*median([RT00; RT11])]);
title('C=1, C_{est}=1, T=[-1.8,1.8]','FontSize',16);
xlabel('time (s)','FontSize',18);
ylabel('number of decisions','FontSize',18);
orient landscape
